...
int main(void){
    ...
    for(int i=0;i<n;i++){
        int x; scanf("%d", &x);
        int found=0;
        for(int j=0;j<5;j++) if(x==primes[j]) { counts[j]++; found=1; break; }
        if(!found) { printf("Valor invalido %d ignorado\n", x); }
    }
    printf("Frequencias:\n");
    for(int j=0;j<5;j++) printf("%d x %d\n", counts[j], primes[j]);
    int imax = 0;
    for(int j=1;j<5;j++) if(counts[j] > counts[imax]) imax = j;
